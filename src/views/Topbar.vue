<template>
  <nav class="navbar navbar-expand-sm shadow topbar">
    ...
    <div class="collapse navbar-collapse px-2" id="navbarSupportedContent">
      <div class="navbar-nav ml-auto align-text-end">
        <span class="nav-item">
        ...
        <span class="nav-item">
          ...
          >
            ...
            <template #reference>
              ...
              >
                <div>
                  <button class="btn btn btn-info w-100" v-if="loggedIn">
                    <h6 class="mx-1 my-1" v-if="loggedIn">
                      {{ userName }}
                    </h6>
                  </button>
                </div>
...
export default defineComponent({
  setup() {
    ...
    return {
      jumpToCreateSpace() {
        if (!loggedIn.value) {
          router.push({ name: "Login", query: { redirect: "CreateSpace" } });
        } else {
